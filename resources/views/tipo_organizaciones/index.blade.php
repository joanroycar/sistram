@section('title', 'Módulo Tipo De Organizaciones')
        ...
        <div class="row">
            <div class="col-md-12">
                <div class="flex-wrap d-flex justify-content-between align-items-center" style="color: black">
                    <div>
                        <h1>Tipo De Organizaciones</h1>
                    </div>
                    @can('tipo-organizacion.create')

                    <div>
                        
                        <a href="{{ route('tipo-organizacion.create') }}" class="btn btn-primary" style="color: white; border-radius: 12px">
            ...
            <div class="card">
                <div class="card-header d-flex justify-content-between">
                    <div class="header-title">
                        <h4 class="card-title"> Lista De Tipo De Organizaciones </h4>
                    </div>
                </div>
                <div class="card-body">
                                ...
                                @foreach ($tipo_organizaciones as $tipo_organizacion)
                                    <tr>
                                        <td>
                                                    ...
                                                    @can('tipo-organizacion.show')

                                                    <a href="{{ route('tipo-organizacion.show', $tipo_organizacion) }}"> <svg width="32"
                                        ...
                                        <td class="text-center">
                                            {{ $tipo_organizacion->siglas }}
                                        </td>
                                        <td class="text-center">
                                            <ul class="table-controls">
                                                    <form action="{{ route('tipo-organizacion.destroy', $tipo_organizacion) }}" method="POST"
                                                    class="casino formulario">
                                                    @csrf
                                                    @method('DELETE')
                                                    @can('tipo-organizacion.edit')

                                                    <a href="{{ route('tipo-organizacion.edit', $tipo_organizacion) }}" class="bs-tooltip"
                                                        ...
                                                        @can('tipo-organizacion.destroy')
                                                            <button type="submit"
                                                            class="btn btn-danger {{ $tipo_organizacion->status == '2' ? 'disabled' : '' }}"
                                        ...
                                        <td class="text-center">
                                            <ul class="table-controls">
                                                <form action="{{ route('tipo-organizacion.activar', $tipo_organizacion) }}" method="POST"
                                                    class="casino formulario2">
                                                    @csrf
                                                    @method('POST')
                                                    <button type="submit"
                                                        class="btn btn-success {{ $tipo_organizacion->status == '1' ? 'disabled' : '' }}"
