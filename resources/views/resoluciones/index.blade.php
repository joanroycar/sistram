@section('title', 'Módulo Resoluciones')
        ...
        <div class="row">
            <div class="col-md-12">
                <div class="flex-wrap d-flex justify-content-between align-items-center" style="color: black">
                    ...
                    @can('resolucion.create')

                    <div>
                        
                        <a href="{{ route('resolucion.create') }}" class="btn btn-primary" style="color: white; border-radius: 12px">
            ...
            <div class="card">
                <div class="card-header d-flex justify-content-between">
                    <div class="header-title">
                        <h4 class="card-title"> Lista De Resoluciones </h4>
                    </div>
                </div>
                <div class="card-body">
                                ...
                                @foreach ($resoluciones as $resolucion)
                                    <tr>
                                        <td>
                                                    {{ $resolucion->id }}
                                               @can('resolucion.show')
                                                    <a href="{{ route('resolucion.show', $resolucion) }}"> <svg width="32"
                                        ...
                                        <td>
                                            {{ $resolucion->nombre }}</td>

                                       
                                        <td class="text-center">
                                            <ul class="table-controls">
                                                    <form action="{{ route('resolucion.destroy', $resolucion) }}" method="POST"
                                                    class="casino formulario">
                                                    @csrf
                                                    @method('DELETE')
                                                        @can('resolucion.edit')

                                                        <a href="{{ route('resolucion.edit', $resolucion) }}" class="bs-tooltip"
                                                        ...
                                                        @can('resolucion.destroy')
                                                            ...
                                                            <button type="submit"
                                                            class="btn btn-danger {{ $resolucion->status == '2' ? 'disabled' : '' }}"
                                        ...
                                        <td class="text-center">
                                            <ul class="table-controls">
                                                <form action="{{ route('resolucion.activar', $resolucion) }}" method="POST"
                                                    class="casino formulario2">
                                                    @csrf
                                                    @method('POST')
                                                    <button type="submit"
                                                        class="btn btn-success {{ $resolucion->status == '1' ? 'disabled' : '' }}"
