            ...
            <div class="col-md-12">
                <div class="flex-wrap d-flex justify-content-between align-items-center" style="color: black">
                    ...
                    <div>
                        ...
                        <a href="{{ route('role.create') }}" class="btn btn-primary" style="color: white; border-radius: 12px">
                            ...
                            <tbody>
                                @foreach ($roles as $role)
                                    <tr>
                                        <td>{{ $role->id }}
                                            @can('role.show')
                                            <a href="{{ route('role.show', $role) }}"> <svg width="32"
                                        ...
                                        <td class="text-center"><ul class="table-controls">
                                            <form action="{{route('role.destroy', $role)}}" method="POST" class="casino">
                                                @csrf
                                                @method('DELETE')
                                                @can('role.edit')
                                                <a href="{{route('role.edit',$role)}}" class="btn btn-primary" data-bs-toggle="tooltip" data-bs-placement="top" title="Editar" data-original-title="Edit"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-edit-2 p-1 br-8 mb-1"><path d="M17 3a2.828 2.828 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5L17 3z"></path></svg></a>
                                                @endcan
                                                @can('role.destroy')
                                        ...
                                        {{-- <td class="text-center">
                                            <ul class="table-controls">
                                                <form action="{{ route('usuario.activar', $categorium) }}" method="POST"
