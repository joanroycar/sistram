    ...
    <div class="row">
        <div class="col-sm-12 col-lg-12">
            <div class="card">
                ...
                <div class="card-body">
                    {!! Form::open(['route' => 'persona.store', 'autocomplete' => 'off', 'files' => true, 'class' => 'formulario']) !!}
                    
                    <div class="form-group">
                        <label class="form-label" for="pwd">Nombres:</label>
                        <input type="text" class="form-control" id="nombres" name="nombres"
                            placeholder="Ingrese El nombre" value="{{ old('nombres') }}" onkeypress="return soloLetrasTildesNombreCampo(event)" required>
                        @error('nombres')
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="apellidos" name="apellidos"
                            placeholder="Ingrese El apellido" value="{{ old('apellidos') }}" onkeypress="return soloLetrasTildesNombreCampo(event)" required>
                        @error('apellidos')
                            <strong class="text-sm text-red-600" style="color: red">{{ $message }}</strong>
                        @enderror
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select name="tipo_documento_id" class="form-control" id="tipo_documento_id">
                            <option value="" selected disabled>Seleccione un Tipo De Documento......</option>
                            @foreach ($tipoDocumentos as $tipoDocumento)
                                <option value="{{ $tipoDocumento->id }}">{{ $tipoDocumento->nombre }} - {{ $tipoDocumento->siglas }}</option>
                            @endforeach
                        </select>
                    ...
                    <input type="text" class="form-control" id="cedula" name="cedula"
                            ...
                            placeholder="Ingrese la cedula" value="{{ old('cedula') }}" onkeypress="return solonumeros(event)" minlength="8" maxlength="11" required>                       
                             @error('cedula')
                            <strong class="text-sm text-red-600" style="color: red">{{ $message }}</strong>
                        @enderror
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="telefono" name="telefono"
                            placeholder="Ingrese El Telefono" value="{{ old('telefono') }}" onkeypress="return solonumeros(event)">
                        @error('telefono')
                            <strong class="text-sm text-red-600" style="color: red">{{ $message }}</strong>
                        @enderror
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="domicilio" name="domicilio"
                            placeholder="Ingrese El Domicilio" value="{{ old('domicilio') }}" >
                        @error('domicilio')
                            <strong class="text-sm text-red-600" style="color: red">{{ $message }}</strong>
                        @enderror
                    </div>
...
$("#tipo_documento_id").on("change",function(){
 ...
 var numdoc_sel = $("#cedula");
 if(tdoc_value == "1"){
  numdoc_sel.attr("minlength","8");
  numdoc_sel.attr("maxlength","8");
  numdoc_sel.attr("onlynumber","true");
 }else if(tdoc_value == "3"){
  numdoc_sel.attr("minlength","9");
  numdoc_sel.attr("maxlength","9");
  numdoc_sel.attr("onlynumber","true");
 }else if(tdoc_value == "2"){
  numdoc_sel.attr("minlength","12");
