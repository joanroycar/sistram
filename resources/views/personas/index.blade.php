            ...
            <div class="col-md-12">
                <div class="flex-wrap d-flex justify-content-between align-items-center" style="color: black">
                    ...
                    <div>
                        ...
                        <a href="{{ route('persona.create') }}" class="btn btn-primary" style="color: white; border-radius: 12px">
                                ...
                                @foreach ($personas as $persona)
                                    <tr>
                                        <td>
                                                    {{ $persona->id }}
                                                    @can('persona.show')
                                                    <a href="{{ route('persona.show', $persona) }}"> <svg width="32"
                                        ...
                                        <td>
                                            {{ $persona->empresa->razon_social }}</td>
                                        <td>
                                            {{ $persona->nombres }}</td>

                                        <td>{{ $persona->apellidos }}</td>

                                        <td class="text-center">{{ $persona->tipodocumentos->siglas }}</td>
                                        ...
                                        <td class="text-center">
                                            <ul class="table-controls">
                                                    ...
                                                    @method('DELETE')
                                                    @can('persona.edit')
                                                    <a href="{{ route('persona.edit', $persona) }}" class="bs-tooltip"
                                                        ...
                                                        @endcan
                                                        @can('persona.destroy')
                                                        <button type="submit" class="btn btn-danger {{ $persona->status == '2' ? 'disabled' : '' }}" style="">Dar
                                                            baja</button>
                                                            @endcan
                                        ...
                                        <td class="text-center">
                                            <ul class="table-controls">
                                                <form action="{{ route('persona.activar', $persona) }}" method="POST"
                                                    class="casino formulario2">
                                                    @csrf
                                                    @method('POST')
                                                    <button type="submit"
                                                        class="btn btn-success {{ $persona->status == '1' ? 'disabled' : '' }}"
